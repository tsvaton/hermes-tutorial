                                           ...
                                           double* current_time_ptr, double temp_init, double t_final) : WeakForm<double>(1)
{
  ...
  add_matrix_form(new DefaultJacobianDiffusion<double>(0, 0, new Hermes1DFunction<double>(-lambda / (heatcap * rho))));

  // Jacobian surface part.
  add_matrix_form_surf(new DefaultMatrixFormSurf<double>(0, 0, new Hermes2DFunction<double>(-alpha / (heatcap * rho)), bdy_air));

  // Residual - volumetric.
  add_vector_form(new DefaultResidualDiffusion<double>(0, new Hermes1DFunction<double>(-lambda / (heatcap * rho))));
